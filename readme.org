...
This is for automatic handling of files ending in ~.gpg~, not to be confused with ~org-crypt~.
...
Set tab width to 2 in Web buffers
#+begin_src emacs-lisp
...
** JSON
This configures JSON mode and jsonc-mode
#+BEGIN_SRC emacs-lisp
...
** Org JSON Formatting
Pretty-print JSON within =#+begin_src json= blocks.
- =M-x iw/org-format-json-block= - Format block at point
- =M-x iw/org-format-all-json-blocks= - Format all blocks in buffer
#+begin_src emacs-lisp
...
** Timestamp babel results on execution
When ~:timestamp t~ is set as a header argument on a source block, executing the block inserts a timestamp comment above the ~#+RESULTS:~ line. Adapted from [[https://emacs.stackexchange.com/questions/16850/timestamp-on-babel-execute-results-block][this StackExchange answer]], modernized to use ~advice-add~ and a comment line rather than modifying the ~#+RESULTS~ brackets.
...
** Show the running process when :session is provided
This is slop that I have only lightly reviewed but seems to work.
...
*** Org Babel Support
Kagi FastGPT is also supported in Org Babel blocks, which will be nice if I ever use it and want to capture the responses alongside notes
#+begin_src emacs-lisp
...
** When clicking links, reuse last window
This changes the default behavior so that when following a link, if only one window is rendered, split it and show the destination in the new window. Otherwise, use the last window that was in use before clicking the link, to display the destination.
