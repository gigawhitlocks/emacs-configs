...
*** Git Config
I might as well save my global ~git~ configuration here, as well, since ~magit~ is extra good if ~git~ is well-configured. I selected most of these options from [[https://blog.gitbutler.com/how-git-core-devs-configure-git][this blog post]] but I didn't select all of the options in that post, so I highly recommend it.
...
#+begin_src conf :tangle ~/.config/git/config :comments link
  [column]
    ui = auto
#+end_src
...
Settings for diffs:
- ~histogram~ produces [[https://adamj.eu/tech/2024/01/18/git-improve-diff-histogram/][slightly different diffs from the default]], which I find more readable.
