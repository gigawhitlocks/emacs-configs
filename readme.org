...
[[https://github.com/xenodium/agent-shell][agent-shell]] provides a native Emacs buffer to interact with LLM coding agents via the ACP (Agent Client Protocol). OpenCode has built-in ACP support via ~opencode acp~. Other agents are not currently configured.
