#+COMMENT: -*- mode: org; eval: (add-hook 'after-save-hook '(lambda () (org-gfm-export-to-markdown nil nil nil) (org-babel-tangle)) nil t) -*-
#+TITLE: Emacs 💕
#+TAGS[]: emacs
#+MENU: main
#+DATE: <2019-04-24 Wed>
...
In addition to per-line scrolling (when using the mouse wheel), Emacs also has an antiquated way of deciding when to scroll a document based on the cursor. The default behavior, and a guess at the reason behind it, are well-described [[https://themkat.net/2025/03/25/simple_smoother_emacs_scrolling.html][in the blog post where I encountered the following setting]], which makes the buffer scroll in a way more akin to modern software when not using the mouse wheel:
...
** Dependencies:
[[https://troglobit.com/post/2024-03-03-building-emacs-with-jit/][This helpful blog post]] clued me in to how to figure out which version of ~libgccjit~ is needed; run ~gcc --version~ to see the major version the OS is going to call when calling ~make~ and install ~libgccjit-$VERSION-dev~. Thanks @troglobit!

**  Build the release
 #+BEGIN_SRC shell :dir ~/emacs
   ./autogen.sh
...
- I had to remove support for [[https://www.reddit.com/r/emacs/comments/1fpd3dk/problem_compiling_latest_git_version/][xwidgets due to this issue]] in 30.1. Revisit in the future? Maybe I wasn't using them.
