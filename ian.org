...
Unfortunately, Phind didn't do the job perfectly, so some of these are in the wrong category. I might fix it at some point, I don't know; it doesn't really matter.
...
** Kagi integration
I love Kagi and even if it costs a few cents per query I would like to have it accessible from Emacs. Uses API key stored in ~~/.secret.el~~ [[* Secrets][as configured below]]
*** Basic config
#+begin_src emacs-lisp
  (use-package kagi
    :custom
...
*** TODO The docs mention a package called ~password-store~ that I should look into
