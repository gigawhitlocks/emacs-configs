...
#+BEGIN_SRC bash :tangle ~/.emacs.d/get-irc-password.bash :shebang #!/bin/bash
...
This looks like it's possible but it's going to be hard to make automatic.
I was hoping that I could make use of Firefox Sync but it looks like the setting that needs to be toggled is in ~about:config~.
...
[[https://askubuntu.com/questions/514125/url-protocol-handlers-in-basic-ubuntu-desktop][Use this to add a MIME handler for emacs:// links to open in Emacs]]
