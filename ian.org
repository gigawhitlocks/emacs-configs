...
- ~go install~ [[https://github.com/rogpeppe/godef][~godef~ for definitions]]
- ~gopls~, [[https://github.com/golang/tools/blob/master/gopls/doc/user.md][the language server for LSP mentioned above]]
- ~gocode~ just do ~go get -u github.com/stamblerre/gocode~ and also maybe visit the [[https://github.com/stamblerre/gocode][website]] to see if it's deprecated because this project keeps getting passed around for some reason. I swear every time I install it, it's from a different fork...
