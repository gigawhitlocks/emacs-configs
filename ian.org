...
First, write a new command to get the nick:
(TODO eliminate this dependency by just doing this in elisp directly?)

#+BEGIN_SRC bash :tangle ~/.emacs.d/get-nick.bash :shebang /bin/bash
cat ~/.authinfo | grep "$1" | sed 's/.*login \([^ ]\+ \).*/\1/g'
#+END_SRC

Then configure Emacs to use this to find the nick:
