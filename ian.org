...
***  add a command for manual formatting w/ Prettier
IDK, for some reason the auto-formatter wasn't working for me so I had Phind generate a command for me to call the command interactively whenever I feel like. This command replaces the current buffer with formatted text.
#+begin_src emacs-lisp
  (defun ian-prettier-format ()
    ...
    (let ((file-name (buffer-file-name))
      ...
      (let ((formatted-output (shell-command-to-string (format "yarn prettier %s" file-name))))
        (erase-buffer)
        (insert formatted-output)
        (goto-char current-point))))
#+end_src
*** automatically format on save
at least until I figure out why prettier-js-mode isn't working, I can just call ian-prettier-format on save
#+begin_src emacs-lisp
  (add-hook 'web-mode-hook
            (lambda ()
              (add-hook 'before-save-hook 'ian-prettier-format nil t)))
#+end_src
